...
begin
  if NameIsInRule(lbRules.ItemIndex,cbModels.Text) then
    ShowMessage('Taki pojazd jest już w tej regule.')
  else
...
begin
  if Util.Ask('Usunąć wybrany element reguły?'{Util.LabelStr(CAP_SET_CHANGED)}) then
  begin
...
begin
  if Util.Ask('Zapisać ustalone reguły?'{Util.LabelStr(CAP_SET_CHANGED)}) then
    SaveRules;
end;
