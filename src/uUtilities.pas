...
type
  TLabels = (LAB_CAR_NO, LAB_AI_TRAIN, LAB_KEY1, LAB_KEY2, LAB_KEY3,
    LAB_KEY_DESC, LAB_WRONG_CONNECTION, LAB_TRAIN_NAME, LAB_TRAIN_NAME_CHANGE,
    LAB_FILE_NOT_FOUND, LAB_SAVE_PRESET, LAB_SET_PRESET_NAME,
    LAB_RANDOM_TEX_EXCEPT,

    LAB_LOAD_SETTINGS, LAB_LOAD_SCENERIES, CAP_START_NO_EXE, CAP_START_NO_SCN,
    CAP_START_GO_SEL_TRAIN, CAP_START_SEL_TRAIN, CAP_START_SEL_VEHICLE,
    CAP_START_NO_STAFF, CAP_NO_VEHICLES,

    LOG_NOT_FOUND_EXE, LOG_INTERNAL_ERROR, LOG_DEPO_LOAD_EXCEPT,
    LOG_SCN_MINI_LOAD_FAULT,

    LOG_TEX_NO_FILE, LOG_TEX_NO_MODEL, LOG_TEX_NO_PHYSICS,
    LOG_TEX_NO_MULTIMEDIA,

    LOG_LOAD_SCN, LOG_PARSE_SCN,

    LOG_NO_FILE, LOG_NO_MODEL, LOG_NO_PHYSICS, LOG_NO_MULTIMEDIA,
    LOG_CHECK_VALUE_FAULT, LOG_CHECK_PHYSICS_FILE, LOG_UNSUPPORTED_LOAD,
    ...
    CAP_ALGORITHM_FAULT, CAP_SET_CHANGED, CAP_FILE_NOT_FOUND,
    CAP_LOAD_MINI_FAULT, CAP_REMOVE_OLD_VER_FAULT, CAP_YES, CAP_NO, CAP_NO_DIR,
    CAP_NO_WEIGHTS, CAP_NO_SCN, CAP_NO_VEHICLES_2, CAP_NO_PHYSICS, CAP_NO_EXE,
    CAP_ERRORS, CAP_PROGRAM_FILES, CAP_CREATE_FILE_FAULT, CAP_LOGO_FAULT,
    CAP_CURRENT_VERSION, CAP_NEWER_VERSION_ASK, CAP_UPDATE_FAULT,
    CAP_UPDATE_FAULT_EXT, CAP_UPDATED_PROGRAM, CAP_UPDATING,
    CAP_OPERATION_FAULT, CAP_REMOVE_ALL_VEHICLES, CAP_LOADING_DEPOT,
    CAP_LOADING_PHYSICS, CAP_LOADING_SCN, CAP_LOADING_WEIGHTS,
    CAP_LOAD_MODELS_FAULT, CAP_LOAD_SCN_FAULT, CAP_PARSE_TRAIN_FAULT,
    CAP_VEHICLE_FAULT, CAP_LOAD_WEIGHTS_FAULT, CAP_VEHICLE_SYNTAX_FAULT,
    CAP_VEHICLE_PARSE_FAULT, CAP_STRINT_SYNTAX_FAULT, CAP_STRFLOAT_SYNTAX_FAULT,
    CAP_CONFIG_PARSE_FAULT, CAP_ATMO_PARSE_FAULT, CAP_PARSE_SCN,
    CAP_PARSE_SCN_FAULT, CAP_PARSE_TRAINSET_FAULT, CAP_PARSE_TEXTURES_FAULT,
...
uses Winapi.ShellApi, Vcl.Forms, Winapi.Windows, Vcl.Graphics, SysUtils,
  Dialogs, JPEG, uMain,
...
procedure OpenDir(const Path: string);
begin
  ShellExecute(Application.Handle, PChar('explore'), PChar(Path), nil, nil,
    SW_SHOWNORMAL);
end;
...
begin
  if FileExists(Util.Dir + Path) then
    ShellExecute(Application.Handle, 'open', PChar(Util.Dir + Path), nil, nil,
      SW_SHOWNORMAL)
  else
...
begin
  try
    ...
    else if FileExists(Dir + 'textures\mini\' + Model.Mini + '.bmp') then
...
begin
  try
    ...
    on E: Exception do
      Util.Log.Add(Format(Util.LabelStr(CAP_REMOVE_OLD_VER_FAULT),
        [E.Message]));
  end;
end;
...
begin
  try
    ...
    Util.Log.Add(Format(LabelStr(CAP_CREATE_FILE_FAULT),
      ['dynamic\textures.ini']));
  end;
end;
...
begin
  ...
  try
    try
      ...
      begin
        FoundFiles := FindFirst(Util.Dir + 'textures\logo\logo*.jpg',
          faAnyFile, SR);
        ...
        JPG.LoadFromFile(Util.Dir + 'textures\logo\' + FilesList
          [Random(FilesList.Count)]);
      end;
...
function CompareVehicleNames(const Item1, Item2: Pointer): Integer;
begin
  if (TTrain(Item1).Vehicles.Count > 0) and (TTrain(Item2).Vehicles.Count > 0)
  then
    Result := CompareText(TTrain(Item1).Vehicles[0].Name,
      TTrain(Item2).Vehicles[0].Name)
  else
    Result := -1;
end;
...
begin
  LabelsArray[Ord(LAB_TRAIN_NAME), 1] := 'Nazwa pociągu';
  LabelsArray[Ord(LAB_TRAIN_NAME_CHANGE), 1] := 'Zmiana nazwy pociągu';
  LabelsArray[Ord(LAB_CAR_NO), 1] := 'Numer wagonu:';
  LabelsArray[Ord(LAB_AI_TRAIN), 1] := 'Pociąg prowadzony przez komputer.';
  ...
  LabelsArray[Ord(LAB_SAVE_PRESET), 1] := 'Zapis presetu ustawień';
  LabelsArray[Ord(LAB_SET_PRESET_NAME), 1] := 'Nadaj nazwę zestawu ustawień:';
  LabelsArray[Ord(LAB_RANDOM_TEX_EXCEPT), 1] :=
    'Wystąpił błąd przy losowaniu tekstur. Szczegóły błędu:';

  LabelsArray[Ord(LAB_LOAD_SETTINGS), 1] := 'Wczytywanie ustawień...';
  ...
  LabelsArray[Ord(CAP_START_GO_SEL_TRAIN), 1] := 'Przejdź do wyboru składu';
  LabelsArray[Ord(CAP_START_SEL_TRAIN), 1] := 'Wybierz skład do prowadzenia';
  ...
  LabelsArray[Ord(LOG_INTERNAL_ERROR), 1] := 'Błąd wewnętrzny Startera.';
  LabelsArray[Ord(LOG_DEPO_LOAD_EXCEPT), 1] :=
    'Błąd wczytywania magazynu. Szczegóły błędu:';
  LabelsArray[Ord(LOG_SCN_MINI_LOAD_FAULT), 1] :=
    'Nie udało się wczytać miniaturki scenariusza';
  ...
  LabelsArray[Ord(LOG_TEX_NO_MULTIMEDIA), 1] :=
    'Brak pliku mulitmediów dla tekstury';

  LabelsArray[Ord(LOG_LOAD_SCN), 1] := 'Ładowanie scenerii';
  ...
  LabelsArray[Ord(LOG_CHECK_VALUE_FAULT), 1] := 'Błąd sprawdzania wartości';
  LabelsArray[Ord(LOG_CHECK_PHYSICS_FILE), 1] :=
    'Należy sprawdzić plik .fiz dla';
  LabelsArray[Ord(LOG_UNSUPPORTED_LOAD), 1] :=
    'zastosowany nieobsługiwany ładunek przez pojazd';

  LabelsArray[Ord(LOG_DEPO_PARSE_FAULT), 1] :=
    'Błąd parsowania magazynu. Linia:';
  LabelsArray[Ord(LOG_DEPO_SAVE_FAULT), 1] := 'Błąd zapisu magazynu.';
  LabelsArray[Ord(LOG_PHYSICS_PARSE_FAULT), 1] :=
    'Błąd prztwarzania elementu fizyki. Token:';

  LabelsArray[Ord(CAP_LOAD_SETTINGS_FAULT), 1] :=
    'Błąd wczytywania ustawień (plik %s).';
  LabelsArray[Ord(CAP_PARAMETER), 1] := 'Parametr:';
  LabelsArray[Ord(CAP_INVALID_VALUE), 1] := 'Błędna wartość:';
  LabelsArray[Ord(CAP_FAULT_DETAIL), 1] := 'Szczegóły błędu:';
  LabelsArray[Ord(CAP_ALGORITHM_FAULT), 1] :=
    'Wystąpił błąd przy próbie zmiany algorytmu. Szczegóły błędu:';
  LabelsArray[Ord(CAP_SET_CHANGED), 1] :=
    'Wykryto zewnętrzne zmiany w ustawieniach symulatora. Czy wczytać ustawienia ponownie?';

  LabelsArray[Ord(CAP_FILE_NOT_FOUND), 1] := 'Nie znaleziono pliku: %s';
  LabelsArray[Ord(CAP_LOAD_MINI_FAULT), 1] :=
    'Nie udało się wczytać miniaturki pojazdu %s';
  LabelsArray[Ord(CAP_REMOVE_OLD_VER_FAULT), 1] :=
    'Nie udało się usunąć poprzedniej wersji Startera. Szczegóły błędu: %s';

  LabelsArray[Ord(CAP_YES), 1] := 'Tak';
  LabelsArray[Ord(CAP_NO), 1] := 'Nie';
  LabelsArray[Ord(CAP_NO_DIR), 1] := 'Brak katalogu %s';
  LabelsArray[Ord(CAP_NO_WEIGHTS), 1] := 'Brak informacji o wagach ładunków.';
  ...
  LabelsArray[Ord(CAP_ERRORS), 1] := 'Możliwa błędna instalacja symulatora.';
  LabelsArray[Ord(CAP_PROGRAM_FILES), 1] :=
    'Program zainstalowany w katalogu Program Files.';
  LabelsArray[Ord(CAP_CREATE_FILE_FAULT), 1] :=
    'Nie udało się utworzyć pliku: %s';
  LabelsArray[Ord(CAP_LOGO_FAULT), 1] :=
    'Błąd obsługi logo. Szczegóły błędu: %s';
  LabelsArray[Ord(CAP_CURRENT_VERSION), 1] := 'Posiadasz najnowszą wersję.';
  LabelsArray[Ord(CAP_NEWER_VERSION_ASK), 1] :=
    'Dostępna jest nowsza wersja. Zaktualizować program?';
  LabelsArray[Ord(CAP_UPDATE_FAULT), 1] := 'Błąd aktualizacji';
  LabelsArray[Ord(CAP_UPDATE_FAULT_EXT), 1] :=
    'Wystąpił błąd podczas aktualizacji programu.';
  LabelsArray[Ord(CAP_UPDATED_PROGRAM), 1] := 'Program został zaktualizowany.';
  LabelsArray[Ord(CAP_UPDATING), 1] := 'Aktualizuję...';
  LabelsArray[Ord(CAP_OPERATION_FAULT), 1] := 'Wystąpił błąd podczas operacji.';
  LabelsArray[Ord(CAP_REMOVE_ALL_VEHICLES), 1] :=
    'Usunąć wszystkie pojazdy na scenerii?';
  ...
  LabelsArray[Ord(CAP_LOADING_WEIGHTS), 1] := 'Wczytywanie ładunków...';
...
function TUtil.GetFileVersion(const FileName: string;
  StrFormat: string = '%d.%d.%d'): string;
var
