...
begin
  ...
  LabelsArray[Ord(LAB_RANDOM_TEX_EXCEPT), 1] :=
    'Wyst¹pi³ b³¹d przy losowaniu tekstur. Szczegó³y b³êdu:';
  ...
  LabelsArray[Ord(LOG_DEPO_LOAD_EXCEPT), 1] :=
    'B³¹d wczytywania magazynu. Szczegó³y b³êdu:';
  LabelsArray[Ord(LOG_SCN_MINI_LOAD_FAULT), 1] :=
    'Nie uda³o siê wczytaæ miniaturki scenariusza';
  ...
  LabelsArray[Ord(CAP_ALGORITHM_FAULT), 1] :=
    'Wyst¹pi³ b³¹d przy próbie zmiany algorytmu. Szczegó³y b³êdu:';
  LabelsArray[Ord(CAP_SET_CHANGED), 1] :=
    'Wykryto zewnêtrzne zmiany w ustawieniach symulatora. Czy wczytaæ ustawienia ponownie?';

  LabelsArray[Ord(CAP_FILE_NOT_FOUND), 1] := 'Nie znaleziono pliku: %s';
  LabelsArray[Ord(CAP_LOAD_MINI_FAULT), 1] :=
    'Nie uda³o siê wczytaæ miniaturki pojazdu %s';
  LabelsArray[Ord(CAP_REMOVE_OLD_VER_FAULT), 1] :=
    'Nie uda³o siê usun¹æ poprzedniej wersji Startera. Szczegó³y b³êdu: %s';
  ...
  LabelsArray[Ord(CAP_REMOVE_ALL_VEHICLES), 1] :=
    'Usun¹æ wszystkie pojazdy na scenerii?';
